/** Problema 1 (P4): Implementa una nueva operación del TAD Abb que tomando un elemento del mismo
...
//Método para podar un subarbol
template <typename T>
void podaSubArbol(const T& e, const Abb<T>& A){

    Abb<T> B;

    if(A != A.vacio()){

        if(B.buscar(e) != B.vacio()){
            eliminar(e);
            A.podaSubArbol(e, A.izqdo());
            A.podaSubArbol(e, A.drcho());
        }
    }
}
