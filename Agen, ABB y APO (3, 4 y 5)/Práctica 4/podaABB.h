...
template <typename T> class Abb {
    ...
    public:
        ...
        void podar(const T& e); //Método para podar subárboles a partir del elemento e.
...
//Método para podar subarbol
template <typename T>
void podar(const T& elemento){

    Abb A;
    A = buscar(elemento);

    if(A == nullptr)
        cout << "El elemento introducido no se encuentra en el arbol." << endl;
    else
